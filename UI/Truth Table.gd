...
func HandleCalculation():
	...
	for i in array.Variables.keys():
		...
		head.text=i
		head.custom_minimum_size.x=$ScrollContainer/GridContainer.custom_minimum_size.x/$ScrollContainer/GridContainer.columns
	...
	for i in range(pow(2,array.Variables.size())):
		...
		for j in range($ScrollContainer/GridContainer.columns):
			...
			label.custom_minimum_size.x=$ScrollContainer/GridContainer.custom_minimum_size.x/$ScrollContainer/GridContainer.columns
